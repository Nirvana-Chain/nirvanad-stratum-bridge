...
func main() {
	...
	cfg := nirvanatratum.BridgeConfig{}
	...
	flag.StringVar(&cfg.RPCServer, "nirvana", cfg.RPCServer, "address of the Nirvanad node, default `localhost:22110`")
	...
	log.Printf("\tNirvanad:        %s", cfg.RPCServer)
	...
	if err := nirvanatratum.ListenAndServe(cfg); err != nil {
		log.Println(err)
	}
}
