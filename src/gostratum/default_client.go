...
func CleanWallet(in string) (string, error) {
	...
	if !strings.HasPrefix(in, "nirvana:") {
		return CleanWallet("nirvana:" + in)
	}

	// has nirvana: prefix but other weirdness somewhere
	if walletRegex.MatchString(in) {
		return in[0:67], nil
	}
	return "", errors.New("unable to coerce wallet to valid nirvana address")
}
